    ...
    def _wrapper(*args):
        ...
        def modify_task(task):
            ...
            if task and task["basename"] != "_sdk_tools":
                ...
                task.setdefault("uptodate", []).append(
                    codasip.utility.utils.config_changed(str(sdk_id))
                )

                if model.ia:
                    task.setdefault("task_dep", []).append(
                        model.get_task_id("_sdk_tools")
                    )
    ...
    def _wrapper(*args):
        ...
        def modify_task(task):
            ...
            if task and task["basename"] != "_hdk_tools":
                ...
                task.setdefault("uptodate", []).append(
                    codasip.utility.utils.config_changed(str(hdk_id))
                )
...
def task_compile_commands(model):
    ...
    :rtype: dict
    """
    builder = tasks.CompileCommandsBuilder(model)

    def title(task):
        return "Generate compile_commands.json"

    return {
        "basename": "_compile_commands",
        "actions": [builder.run],
        "targets": builder.targets,
        "file_dep": builder.file_dep,
        "uptodate": [codasip.utility.utils.config_changed(builder.uptodate)],
        "title": title,
    }
...
def task_hdk_tools(model):
    ...
    required_libs_dst = [
        os.path.join(contrib_dir, os.path.basename(f)) for f in required_libs_src
    ]
    ...
    return {
        ...
        "basename": "_hdk_tools",
        "actions": [
            (copy_hdk_tools, [files]),
            (copy_contrib, [bin_dir, contrib_dir]),
        ],
...
@sdk_generator
def task_sim_gentool(model):
    """Generate simulator's source files."""

    sim = tasks.SimulatorGentool(model)
    ia_model = model.get_sibling_model(ia=True).filter_model()
    dsm_task = ia_model.get_task_id("dsm")

    def title(task):
        return "Simulator Generation"

    task = {
        "actions": [sim.run],
        "basename": "_sim_gentool",
        "targets": sim.targets,
...
def task_sim_build(model):
    ...
    task = {
        ...
        "targets": sim.targets,
        "file_dep": sim.file_dep,
        "task_dep": [model.get_task_id("_sim_gentool")],
        "uptodate": [codasip.utility.utils.config_changed(sim.uptodate)],
        "title": title,
    }
    return task
...
def task_generate_sim(model):
    """Group task that aggregates the generation of simulator
    source files and the build of the simulator itself."""
    return {
        "basename": "sim",
        "actions": [],
        "task_dep": [
            model.get_task_id("_sim_gentool"),
            model.get_task_id("_sim_build"),
        ],
        "title": lambda _: "Simulator",
    }
...
def task_extract_semantics(model, type):
    ...
    return {
        ...
        "uptodate": [
            codasip.utility.utils.config_changed(semantics_extractor.uptodate)
        ],
        "title": title,
    }
...
def task_include(model):
    ...
    compiler_include_dir = os.path.join(
        str(model.work_dir), "ia", "compiler", "compiler"
    )
    inlines_h = os.path.join(compiler_include_dir, "inlines.h")
    uptodate += [
        codasip.utility.utils.folder_changed(compiler_include_dir, includes="*.h")
    ]
...
def task_compiler(model):
    ...
    task_dep = ["_backend", "_include", "_llc"]
...
def task_build_libs(model):
    ...
    for f in libs.tasks:
        ...
        def title(task):
            name, configuration = f.get_name_and_configuration(task["basename"])
            return f"{name} library, configuration string: {configuration}"
...
def task_isa_visualization(model):
    ...
    task = {
        ...
        "uptodate": [
            codasip.utility.utils.config_changed(isa.uptodate),
        ...
        "params": [
            {
                ...
                "type": list,
                "default": [],
                "help": "Optional preprocessor defines for constraintgen. Use --define='OPTION[=VALUE]' arguments to pass defines.",
            }
        ],
        "targets": [],
        "task_dep": [model.get_task_id(t) for t in sdk_tasks],
...
def task_generate_doc(model):
    ...
    return {
        ...
        "uptodate": [
            codasip.utility.utils.config_changed(docs.uptodate),
...
@codasip.utility.filter_decorator.filter(ia=False)
def task_generate_rtl(model):
    ...
    ca_model = model.get_sibling_model(ia=False).filter_model()
    return {
        "basename": "rtl",
        "actions": [],
        "task_dep": [
            ca_model.get_task_id("_rtl_sources"),
            ca_model.get_task_id("_rtl_ocd_sw"),
            ca_model.get_task_id("_rtl_dbg"),
        ],
        "title": lambda _: "RTL",
    }
...
@hdk_generator
@codasip.utility.filter_decorator.filter(ia=False)
def task_rtl_sources(model):
    rtl = tasks.RtlSourceBuilder(model)
    ca_model = model.get_sibling_model(ia=False).filter_model()

    return {
        "basename": "_rtl_sources",
        "actions": [rtl.run],
        "targets": rtl.targets,
        "file_dep": rtl.file_dep,
        "task_dep": [ca_model.get_task_id("model")],
        "uptodate": rtl.uptodate_deps,
        "title": lambda _: "RTL Sources",
    }


@hdk_generator
@codasip.utility.filter_decorator.filter(ia=False)
def task_rtl_ocd_sw(model):
    rtl = tasks.RtlOCDBuilder(model)
    ca_model = model.get_sibling_model(ia=False).filter_model()

    return {
        "basename": "_rtl_ocd_sw",
        "actions": [rtl.run],
        "targets": rtl.targets,
        "file_dep": rtl.file_dep,
        "task_dep": [ca_model.get_task_id("_rtl_sources")],
        "uptodate": rtl.uptodate_deps,
        "title": lambda _: "RTL OCD build",
    }


@hdk_generator
@codasip.utility.filter_decorator.filter(ia=False)
def task_rtl_dbg(model):
    rtl = tasks.RtlDebuggerBuilder(model)
    ia_model = model.get_sibling_model(ia=True).filter_model()
    ca_model = model.get_sibling_model(ia=False).filter_model()
    ...
    return {
        ...
        "targets": rtl.targets,
        "file_dep": rtl.file_dep,
        "task_dep": [ia_model.get_task_id("dbg"), ca_model.get_task_id("_rtl_sources")],
        "uptodate": rtl.uptodate_deps,
        "title": lambda _: "RTL Debugger",
    }
...
def task_generate_uvm(model):
    ...
    if uvm.skip:
        codasip.utility.logger.warning(
            "Cannot find IA model for generating auto UVM for ", uvm.id
        )
        return
    ...
    return {
        ...
        "task_dep": [dsm_task] if uvm.auto else [],
        "uptodate": [
            codasip.utility.utils.config_changed(uvm.uptodate),
            codasip.utility.utils.folder_changed(
                uvm.output_dir,
                includes=[
                    "*" + hdl_ext,
                    "*.f",
                    ...
                    "*.tcl",
                    "*.py",
                    "*" + codasip.utility.utils.get_static_lib_extension(),
                    "*" + codasip.utility.utils.get_shared_lib_extension(),
                ],
            ),
            codasip.utility.utils.folder_changed(
                uvm.rtl_dir, includes=["*" + hdl_ext, "*.f", "*.cfg", "*.json"]
            ),
        ],
        "title": title,
    }
...
def task_publish_ip(project):
    ...
    cosim_include = project.config["publication.cosim"]
    ...
    for id in model_design_paths:
        ...
        if not model:
            codasip.utility.logger.warning(
                "Model {} for publication was not found, skipping.", id
            )
            continue
        ...
        if cosim_include:
            task_dep.append(model.get_task_id("cosim"))
    ...
    def publish_doc(publish_path):
        ...
        for root, dirs, files in os.walk(path, followlinks=True):
            ...
            for fname in files:
                ...
                if fname in [".gitignore"] or codasip.get_extension(fname) in [
                    ".pyo",
                    ".pyc",
                ]:
                    continue
                ...
                codasip.utility.utils.copy(
                    os.path.join(root, fname), os.path.join(target, fname)
                )
    ...
    def publish_ip(publish_path, models):
        ...
        if target_sdk.models:
            target_sdk.save(
                path=os.path.join(publish_path, "sdk"),
                custom_id=model.config["sdk-id"],
            )
        if target_hdk.models:
            target_hdk.save(
                path=os.path.join(publish_path, "hdk"),
    ...
    def copy_licensed_files(publish_path):
        """
        Copy licensed files generated by bootstrap to final IP package
        :param publish_path: path where to publish final package
        :return:
        """
        # Initial checks

        if not os.path.isdir(codasip.license_files_root):
            codasip.utility.logger.warning(
                "Skipping copying of licensed sources as studio was built without them! Do not share built package"
            )
            return

        dst_dir = os.path.join(
            publish_path, os.path.basename(codasip.license_files_root)
        )

        # if dst dir exists, overwrite it with new version. Checking diffs would be slower.
        if os.path.isdir(dst_dir):
            codasip.utility.logger.info(
                "Found existing third-party source code packages. Overwriting!"
            )
            codasip.utility.utils.rmtree(dst_dir)

        codasip.utility.logger.info(
            "Copying third-party source packages to the final IP package"
        )
        codasip.utility.utils.copy(codasip.license_files_root, dst_dir)

    def copy_cosimulation_libraries(publish_path, publish_models):
        """Copies cosumilation libraries to the final IP package"""
        codasip.utility.logger.info(
            "Copying cosimulation libraries to the final IP package"
        )
        # Do this for both IA and CA
        for model in publish_models:
            # Get location of cosimulation libraries
            cosim_libraries = os.path.join(
                str(model.work_dir), "ia" if model.ia else "ca", "cosimulator"
            )
            # create target_path - final_publish_path/cosimulator/<ia,ca>/*
            target_directory = os.path.join(
                publish_path, "cosimulator", "ia" if model.ia else "ca"
            )
            if not os.path.isdir(target_directory):
                codasip.utility.utils.makedirs(target_directory, exist_ok=True)
            shutil.copytree(cosim_libraries, target_directory, dirs_exist_ok=True)
    ...
    # Add GPL licensed sources to IP package
    actions += [(copy_licensed_files, [publish_path])]
    # add cosimulation libraries
    if cosim_include:
        actions += [(copy_cosimulation_libraries, [publish_path, publish_models])]
    # add documentation
    if doc_include:
    ...
    return {
        ...
        "targets": targets,
        "uptodate": [
            codasip.utility.utils.config_changed(publish_path),
...
def task_testing(model):
    ...
    def run(paths, sdks, hdks, model_id):
        ...
        suite = Testsuite(test_dir=paths, sdk_dir=sdks, hdk_dir=hdks, work_dir=tmp_dir)
        rc = suite.run(["--model-id=" + model_id])
        if rc != 0:
            codasip.utility.logger.error(
                f"Testing.{model_id} failed with exit code {rc}"
            )
            return False
        return True
...
def task_remove_options(project):
    ...
    model_info = {
        ia: name and os.path.basename(name) for ia, name in project.model_info.items()
    }
    ...
    def copy_file(file_path, target, filters=None, target_file=False, blacklist=[]):
        """
        ...
        :param filters:
        :param target_file:
        :param blacklist: blacklisted options
        """
        if target_file:
        ...
        if file_ext in codasip_ext:
            ...
            if (
                not filters
                or file_ext != ".codal"
                or file_name == "version.codal"
                or (not is_file_empty(file_path, filters))
            ):
                copy_fnc = partial(
                    codasip.utility.utils.copy_without_options,
                    project,
                    blacklist=blacklist,
                )
            else:
                codasip.utility.logger.info(
                    "Empty file '{}' has been removed", target_path
                )
                copy_fnc = None
    ...
    def run(blacklist):
        ...
        copy_tree("model", "share", filters=[prep_ia, prep_ca], blacklist=blacklist)
        ...
        for source in sources:
            copy_tree(source, blacklist=blacklist)
